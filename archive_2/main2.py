...
base_params = dict(
    ...
    # own params
    data_dir = 'data/NST/dataset', # used only to preprocess, has to be parent to all other
    # num_workers = 0,
    fp16 = True
)
...
class DiffWave(pl.LightningModule):
    ...
    def predict_step(self, batch, batch_idx, use_cuda=None):
        with torch.no_grad():
            ...
            # get device
            use_cuda = use_cuda if use_cuda != None else torch.cuda.is_available()
            ...
            for n in tqdm(range(len(alpha) - 1, -1, -1), desc="diffusion step"):
                c1 = 1 / alpha[n]**0.5
...
def get_pretrained_model(model_path, params, use_cuda=None):
...
def main(model_path, params, exp_name):
    ...
    # this is the logic for when to save the model
    checkpoint_callback = ModelCheckpoint(
		dirpath=f'spectrogram/danish/{exp_name}/checkpoints',
    ...
    trainer = pl.Trainer(
        callbacks=[checkpoint_callback], # runs at the end of every train loop
        log_every_n_steps=100,
        max_epochs=5,
        accelerator='gpu',
        # devices=1,
        logger=TensorBoardLogger('spectrogram/danish', name=exp_name),
    )
...
if __name__ == '__main__':
    ...
    exp_name = "teeest" #parser.parse_args().exp_name
    if exp_name is None:
        raise ValueError(f'You have to name the experiment')
    
    params = AttrDict(base_params)
    model_path = 'diffwave-ljspeech-22kHz-1000578.pt' # get pytorch model in directory
    main(model_path, params, exp_name)

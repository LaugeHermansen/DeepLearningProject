...
class DiffusionEmbedding(nn.Module):
    def __init__(self, max_steps, inference_parameters=None):
        super().__init__()
        self.register_buffer('embedding', self._build_embedding(max_steps), persistent=False)
        if inference_parameters is not None:
            lerp_steps = inference_parameters[True][0]
            self.register_buffer('lerp_embedding', self._build_lerp_embedding(lerp_steps), persistent=False)
            self.embedding_map = {t: i for i,t in enumerate(lerp_steps)}
        self.projection1 = nn.Linear(128, 512)
        self.projection2 = nn.Linear(512, 512)

    def forward(self, diffusion_step):
        if diffusion_step.dtype in [torch.int32, torch.int64, int]:
            x = self.embedding[diffusion_step]
        else:
            x = self.lerp_embedding[self.embedding_map[diffusion_step]]
    ...
    def _build_lerp_embedding(self, steps):
        embeddings = []
        for t in steps:
            embeddings.append(self._lerp_embedding(torch.tensor(t)))
...
class DiffWave(pl.LightningModule):
    def __init__(self, params):
        ...
        #inference params
        self.inference_parameters = self.compute_inference_paramters()
        
        self.diffusion_embedding = DiffusionEmbedding(len(params.noise_schedule), self.inference_parameters)
    ...
    def compute_inference_paramters(self):
...
###############
        ...
        inference_parameters[True] = (T, alpha, alpha_cum, beta)

        beta = training_noise_schedule
        alpha = 1 - beta
        alpha_cum = np.cumprod(alpha)

        T = [i for i in range(len(beta))]

        inference_parameters[False] = (T, alpha, alpha_cum, beta)
...
##########
    ...
    def predict_step(self, batch, batch_idx, fast=True, progress_bar=True):      
        ...
        with torch.no_grad():
            ...
            T, alpha, alpha_cum, beta = self.inference_parameters[fast]

            if len(spectrogram.shape) == 2: # Expand rank 2 tensors by adding a batch dimension.
                spectrogram = spectrogram.unsqueeze(0)
            # spectrogram = spectrogram.to(device)
            audio = torch.randn(spectrogram.shape[0], self.params.hop_samples * spectrogram.shape[-1]).to(self.device)
            
            # noise_scale = torch.from_numpy(alpha_cum**0.5).float().unsqueeze(1).to(device)
            iter_values = range(len(alpha) - 1, -1, -1)
            iterator = tqdm(iter_values, desc="reverse diffusion") if progress_bar else iter_values
            for n in iterator:
                c1 = 1 / alpha[n]**0.5
...
def get_pretrained_model(model_path: str, params, checkpoint, save_dir):
    ...
    if checkpoint == True:
        ...
        if len(model_path) != 1:
            raise ValueError(f'Path is ambimgious: {model_path}')
        ...
        raise ValueError('Something went wrong in get_pretrained_model')
...
class SpeechDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self, filenames=None):
        filenames = glob(f'{self.params.data_dir}/**/*.wav', recursive=True) if filenames is None else filenames
...
def main(model_path, params, exp_name, global_seed, checkpoint):
    ...
    save_dir = f'spectrogram/danish/{exp_name}_{global_seed}/checkpoints'
